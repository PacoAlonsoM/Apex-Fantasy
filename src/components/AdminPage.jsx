  ...
  return (
    <div style={{ maxWidth: 700, margin: "0 auto", padding: "52px 28px", position: "relative", zIndex: 1 }}>
      ...
      <div style={{ borderRadius: 16, border: PANEL_BORDER, background: PANEL_BG, padding: 24, marginTop: 20, backdropFilter: "blur(16px)" }}>
        ...
        <p style={{ fontSize: 13, color: "rgba(255,255,255,0.38)", marginBottom: 16 }}>
          Pull the latest stories into `news_articles` before generating a new Race Brief.
        </p>
        <button
          onClick={syncNews}
      ...
      <div style={{ borderRadius: 16, border: PANEL_BORDER, background: PANEL_BG, padding: 24, marginTop: 20, backdropFilter: "blur(16px)" }}>
        <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: "rgba(255,255,255,0.3)", marginBottom: 8 }}>Step 4 — Generate Race Brief</div>
        <p style={{ fontSize: 13, color: "rgba(255,255,255,0.38)", marginBottom: 16 }}>
          Builds one Race Brief using the latest ingested news plus upcoming race context from OpenF1. This is the first layer for AI analytics on the product.
        </p>
        <button
        ...
        >
          {insightLoading ? "Generating Race Brief..." : "Generate Race Brief"}
        </button>
        ...
        {insightResult && (
          <div style={{ marginTop: 10, padding: "12px 16px", borderRadius: 9, background: insightResult.error ? "rgba(239,68,68,0.1)" : "rgba(34,197,94,0.1)", border: `1px solid ${insightResult.error ? "rgba(239,68,68,0.28)" : "rgba(34,197,94,0.28)"}`, fontSize: 13, color: insightResult.error ? "#F87171" : "#34D399" }}>
            {insightResult.error ? `❌ ${insightResult.error}` : `✅ ${insightResult.headline || "Race Brief generated successfully."}`}
          </div>
        )}
      </div>
    </div>
